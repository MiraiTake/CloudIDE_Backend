...
func main() {
	...
	// Обновление и выход (публичные, но требуют refresh token)
	r.HandleFunc("/auth/refresh", refreshToken).Methods("POST")
	r.HandleFunc("/auth/logout", logoutUser).Methods("POST")
	...
	api.HandleFunc("/project/start/{name}", startProjectContainer).Methods("POST")
	...
	api.HandleFunc("/auth/sessions", getUserSessions).Methods("GET")
	api.HandleFunc("/auth/logout-all", logoutAllDevices).Methods("POST")

	// Периодическая очистка истёкших токенов (каждые 24 часа)
	go func() {

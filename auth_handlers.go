...
func refreshToken(w http.ResponseWriter, r *http.Request) {
	...
	if req.RefreshToken == "" {
		http.Error(w, "Refresh token обязателен", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		http.Error(w, "Невалидный или истёкший refresh token", http.StatusUnauthorized)
	...
	var exists bool
	err = db.QueryRow("SELECT EXISTS(SELECT 1 FROM users WHERE id = ?)", rt.UserID).Scan(&exists)
	if err != nil || !exists {
	...
	log.Printf("Access token обновлён для пользователя %s через refresh token", rt.UserID)

	// Возвращаем новый Access Token (Refresh Token остаётся прежним)
...
func loginUser(w http.ResponseWriter, r *http.Request) {
	...
	// Получаем информацию об устройстве и IP
	...
	tokenPair, err := createTokenPair(user.ID, deviceInfo, ipAddress)
	if err != nil {
		log.Printf("Failed to create token pair: %v", err)
...
// POST /auth/logout — выход из системы (отзыв refresh token)
...
// POST /auth/logout-all — выход из всех устройств
func logoutAllDevices(w http.ResponseWriter, r *http.Request) {
	...
	if err := revokeAllUserTokens(userID); err != nil {
		log.Printf("Failed to revoke all tokens: %v", err)
		http.Error(w, "Ошибка отзыва токенов", http.StatusInternalServerError)
		return
	}

	w.WriteHeader(http.StatusOK)
	w.Write([]byte("Выход выполнен на всех устройствах"))
}
